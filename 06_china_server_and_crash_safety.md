...
**翻译方向**：去国服和谐 → 用 zhTW 国际版繁体 → 转简体，**不用**国服 zhCN 和谐译名。

实战规则：

- 优先从未和谐的 `zhTW` 源取基础译名，再转成简体写入 `zhCN`。
- OpenCC 请选择 `tw2s`/`t2s` 一类字符转换；不要用 `tw2sp` 做游戏术语，`tw2sp` 会把「傳送」转成「发送」这类大陆书面词，破坏 D2R 约定译名。
- 如果来源 mod 的 `zhTW` 名字带额外标记（如孔数、轻/中/重、底材等级、最大值范围），移植时要先剥掉这些来源 mod 的展示装饰，只保留译名本体。
- 不要把目标 mod 自己的掉落短标签、颜色码、徽章、图标、吐槽、最大值标注整条覆盖掉。只替换基础译名段，其余展示层保留。
- 转换后必须跑字体覆盖检查；常见 TW 标点如 `‧` 可能不在自定义字体中，通常改成 `·` 更稳。
