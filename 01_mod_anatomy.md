...
> ⚠️ **很多文本/UI 类 mod 根本不碰这层**（不改平衡性）。一旦动它（尤其覆盖 `levels.txt`/`skills.txt`/`uniqueitems.txt`），**CN 端闪退风险极高**——见 `02_golden_rules.md`。
...
| 改数值 / 平衡 / 新符文之语 / 新物品 | excel/*.txt | ⚠️ CN 闪退高风险，优先用注入而非整文件覆盖 |
